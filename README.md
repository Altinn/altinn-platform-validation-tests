...
- [K6/api/tests/resource-registry/README.md](K6/api/tests/resource-registry/README.md) is the worked example of a functional test family: per-environment configuration with env var overrides in `commons.js`, one client factory per token flavour, strict options that fail the run on any failed check or request, content checks in domain-checks, `expectingStatus` for the one call that should answer an error status, and a teardown that deletes what the run created. Start there when adding tests for a client family that has none.
