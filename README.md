...
- Client methods build their request with the helpers in [K6/clients/common/request.js](K6/clients/common/request.js) rather than repeating the boilerplate: `buildUrl(base, query)` appends query parameters (skipping null/undefined, repeating arrays), `requestParams({ endpoint, action, labels, token, json, headers })` builds the k6 `tags` and `headers` in one call, and `jsonBody(body)` serialises an optional body. Keep one method per Swagger operation, keep `endpoint` as the path with `{placeholders}` for path parameters, and keep `labels` as the last positional argument. See [K6/clients/access-management-bff/connection/connection.js](K6/clients/access-management-bff/connection/connection.js) for the shape.
...
- Draw test data one row per iteration rather than looping the fixture inside the test. [K6/helpers.js](K6/helpers.js) has `getItemFromList(rows)`, which walks the rows by `__ITER`, and `segmentData(rows, getNumberOfVUs())`, which splits them across VUs first. A `forEach` over the whole fixture in the test body does the same work every iteration, so the test stops scaling the way a smoke or breakpoint run expects. Give a functional run at least as many iterations as the fixture has rows, or the later rows never get drawn.
...
- [K6/api/tests/resource-registry/README.md](K6/api/tests/resource-registry/README.md) is a worked example of functional tests for one group of clients: per-environment configuration and members as test data under `K6/testdata/`, one client factory per token flavour in `commons.js`, strict options that fail the run on any failed check or request, content checks in domain-checks, and a teardown that deletes what the run created. Start there when adding tests for clients that have none.
