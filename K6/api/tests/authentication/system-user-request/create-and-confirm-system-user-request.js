...
export default function (data) {
    ...
    group("As a vendor, I can request a system user and have the customer approve it", function () {
        ...
        // since a step that fails calls fail(), and the cleanup then has to know
        // how far the iteration got: deleting a system that was never registered
        // only turns one failure into two.
        ...
        try {
            ...
            group("Approve the request as the customer", function () {
                ...
                approved = ApproveSystemUserRequest(
            ...
            // Every iteration makes a system user and a system of its own, so the
            // cleanup belongs here rather than in a teardown, and in a finally so
            // that a failed step still takes back what it managed to create.
            // Without it a scheduled run leaves both behind every fifteen minutes,
            // which is where the piles of stale system users in the test
