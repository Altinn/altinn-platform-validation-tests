...
/**
 * The rows `subject-lookup-forms.js` draws from, from
 * `K6/testdata/access-management/resource-owner/authorized-parties/subject-lookup-forms/<environment>.csv`.
 *
 * Its own setup data rather than a field on {@link SetupData}, since that scenario reads
 * nothing else and so runs in environments the json fixture does not describe. `run-all.js`
 * merges the two.
 *
 * @typedef {object} SubjectLookupFormsSetupData
 * @property {Array<SubjectLookupFormsRow>} subjectLookupForms The rows.
 */

/**
 * One row of the subject lookup forms csv: an organisation and its daglig leder, described
 * by every identifier form the scenario looks them up by. Everything is a string, since
 * that is what a csv column is, and the request builder stringifies the numeric forms
 * anyway.
 *
 * Generated by `testdataGeneration/subject-lookup-forms-data.js`. Both parties answered
 * non-empty when the file was written, which is what the scenario needs and nothing more.
 *
 * @typedef {object} SubjectLookupFormsRow
 ...
 * @property {string} userId Altinn user id of the daglig leder.
 * @property {string} partyId Party id of the daglig leder.
