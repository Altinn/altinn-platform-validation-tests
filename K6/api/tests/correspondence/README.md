...
- `create-and-upload-correspondence-single-user.js` runs the same upload for
  one fixed recipient with an enterprise service-owner token.
...
The tests use the shared delegation CSV for the active environment. It provides
the PID, user ID, party ID, and party UUID required for PDP authorization.
`CORRESPONDENCE_RECIPIENT` must identify a user present in that dataset.
