 ...
 * silently drops resources. The counts are compared next to the identifiers, so a
 * resource that comes back twice is caught as well and not deduplicated away.
 *
...
function CheckSameResources(resources, otherResources, operation, otherOperation) {
    ...
    const success = check(resources, {
        "CheckSameResources - Both operations describe the same resources": () =>
            identifiers.size > 0 &&
            onlyInFirst.length === 0 &&
            onlyInSecond.length === 0 &&
            identifiers.size === resources?.length &&
            otherIdentifiers.size === otherResources?.length,
    });

    if (!success) {
        console.error(`CheckSameResources - ${operation} returned ${resources?.length} resource(s) with ${identifiers.size} identifier(s), ${otherOperation} returned ${otherResources?.length} with ${otherIdentifiers.size}`);
...
function CheckIdentifiersContain(resources, substring, operation) {
    ...
    const success = check(resources, {
        "CheckIdentifiersContain - Every hit has the searched substring in its identifier": (response) =>
...
function CheckResourcesOwnedBy(resources, orgCode, operation) {
    ...
    const success = check(resources, {
        "CheckResourcesOwnedBy - Every hit is owned by the org code that was searched for": (response) =>
            Array.isArray(response) && response.length > 0 && foreign.length === 0,
    });

    if (!success) {
        console.error(`CheckResourcesOwnedBy - ${operation} returned ${resources?.length} hit(s) for '${orgCode}', ${foreign.length} owned by someone else`);
...
function CheckNoResources(resources, operation) {
    const success = check(resources, {
        "CheckNoResources - The search found nothing": (response) =>
