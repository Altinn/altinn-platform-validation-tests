...
function CheckChangesIdentified(changes, operation) {
    ...
    const success = check(changes, {
        [`CheckChangesIdentified - Every change from ${operation} names a resource and when it changed`]: () =>
...
function CheckPageHoldsLimit(changes, expectedCount, operation) {
    ...
    const success = check(changes, {
        [`CheckPageHoldsLimit - ${operation} returns the ${expectedCount} changes the limit asked for`]: () =>
...
function CheckResourcesAppearOnce(pages, operation) {
    ...
    const success = check(pages, {
        [`CheckResourcesAppearOnce - No resource appears twice across the ${operation} pages`]: () =>
...
function CheckValidationErrorForPath(problem, expectedPath, operation) {
    ...
    const success = check(problem, {
        [`CheckValidationErrorForPath - ${operation} names '${expectedPath}' as the problem`]: () =>
