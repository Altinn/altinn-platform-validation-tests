...
/**
 * Replaces the members of an access list.
 *
 ...
 * @returns {{value: AccessListMembershipDtoAggregateVersionVersionedPaginated|null, etag: string|null, status: number}} Access list members as `value`
...
export function AccessListReplaceMembers(
    ...
    const res = expecting(expectedStatus, () => withRetries(
        () => accessListClient.AccessListReplaceMembers(owner, identifier, request, options?.headers ?? {}, labels),
    ...
    const succeed = check(res, {
        [`AccessListReplaceMembers - status code is ${expectedStatus}`]: (r) =>
    ...
    if (!succeed) {
        ...
        return { value: members, etag, status: res.status };
    }
