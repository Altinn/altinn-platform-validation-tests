...
local team_schedule_map =
  if schedule_map != null &&
     std.objectHas(schedule_map, team) &&
     std.objectHas(schedule_map[team], config_file)
  then schedule_map[team][config_file]
  else {};
...
local workflow = if std.length(schedule_entries) > 0 then
  (basic_workflow.generate_basic_workflow(
     workflow_name,
     base_job_name,
     run_name,
   ) + {
...
/*
std.trace(
  //"hasTeam=" + std.toString(std.objectHas(schedule_map, team) + " " + team),
